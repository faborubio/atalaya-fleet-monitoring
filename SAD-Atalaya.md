...
### ADR-014 — Estrategia de demo de portafolio: dos niveles
**Contexto:** el despliegue real (G5b, [AUD-031](./AUDIT.md)) probó que la plataforma corre en GCP, pero el pipeline completo cuesta **~US$70–120/mes encendido** porque **Cloud SQL y Memorystore no escalan a cero**. Para un portafolio se necesita que un reclutador interactúe con el sistema **sin generar una factura sorpresa** y, a la vez, poder **demostrar la infraestructura real** en una entrevista. Hallazgo clave (verificado en código): el modo `Telemetry:Transport=InMemory` (ADR-011) ya ejecuta **todo el camino caliente en un proceso y empuja en vivo por SignalR** (`TelemetryProcessor` → `devicesUpdated`/`alertsRaised`, histórico in-proc, viewport) — sin Postgres/Redis/Pub/Sub.
**Decisión:** ofrecer la demo en **dos niveles** sobre una **base persistente casi gratis** (Artifact Registry + imágenes, bucket del data lake, BigQuery, Pub/Sub en reposo, SAs, secrets, APIs):
- **Nivel 1 — InMemory always-on (~$0, 24/7):** un único Cloud Run con `Telemetry:Transport=InMemory`, `Auth:Mode=Dev` (login pulido con botón de un clic que luce el JWT+RBAC sin pedir credenciales) y un **generador de telemetría server-side vistoso** (`DemoTelemetryGenerator`, único código nuevo: movimiento fluido + alertas a propósito) + SPA en Firebase Hosting (sitio dedicado `atalaya-dashboard`). `min-instances=0` + `cpu_idle=true` → el generador solo produce mientras un WebSocket está abierto (alguien mirando) y el servicio escala a cero cuando no hay nadie ⇒ costo cero en reposo, dentro del free tier.
- **Nivel 2 — stack efímero completo (solo lo que uses):** el Terraform de G5b **partido** en persistente vs efímero (billables: Cloud SQL, Memorystore, VPC connector, Cloud Run); `up` antes de la entrevista, `down` después. Demuestra la infra real (Pub/Sub, DLQ, autoescalado) en la consola de GCP.
**Razón:** cubre los dos públicos sin tensión de costo — el link 24/7 para el 95% que solo hace clic, y el stack real para la entrevista técnica. Reusa el modo InMemory que ya existía (ADR-011) en lugar de mantener infra cara encendida; el único código nuevo es el generador de datos.
**Trade-off:** (a) el Nivel 1 no exhibe la infra real en vivo (se cubre con el Nivel 2 + docs/AUDIT + video); (b) el Nivel 2 exige presencia humana para `up`/`down` y un `apply` de ~15–20 min (Cloud SQL es el cuello); (c) el teardown del Nivel 2 **no debe** tocar la base persistente (al contrario del `destroy` total de G5b, que borró el Artifact Registry). Plan de implementación al detalle en [DEMO.md](./DEMO.md).
...
| 1.0.5 | 2026-06-24 | **G5a** (AUD-025): IaC con **Terraform** (`infra/terraform/`) del plano de control GCP completo (Artifact Registry, SAs mínimas, Pub/Sub+DLQ+IAM, GCS, BigQuery, Cloud SQL, Memorystore+VPC connector, Secret Manager, 2× Cloud Run always-on, Firebase Hosting) — reemplaza el AWS CDK. **Dockerfiles** de api/worker + container-readiness (worker health en `$PORT`, CORS por config). Verificado `terraform validate` + build de imágenes ($0). El `apply` real + E2E en la nube + teardown = **G5b**. |
...
| 1.0.10 | 2026-06-25 | **G5b** (AUD-031): despliegue real en GCP (`fabian-portafolio`) + smoke E2E en vivo; cierra el roadmap del pivote (G0…G5). **ADR-014**: estrategia de demo de portafolio en **dos niveles** (Nivel 1 InMemory always-on ~$0 + Nivel 2 stack efímero) sobre una base persistente casi gratis; plan en [DEMO.md](./DEMO.md). |
